...
/*   Updated: 2016/05/06 17:01:09 by oseng            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			disk(t_vector r, t_figure p, double *t, t_vector eye)
{
	double		tmp;
...
// if (inter.x * inter.x + inter.y * inter.y < 100)
*/
...
int			plane(t_vector r, t_figure p, double *t, t_vector eye)
{
	...
	if (tmp < *t && tmp > 0.00001)
	{
		*t = tmp;
		inter = calcul_ptinter(eye, r, *t);
		if ((pow(inter.z - 1, 2) + pow(inter.y -20, 2)) > 100)
		return (1);
	}
	return (0);
}
