...
/*   Updated: 2016/06/03 12:38:20 by svelhinh         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		mlx_pixel_put_to_image(int color, t_env *img, int x, int y)
{
	...
	if (x < img->w && x > 0 && y < img->h && y > 0)
	{
	    ...
	    g = (color & 0xFF00) >> 8;
	    b = (color & 0xFF);
 		img->data[y * img->line + x * img->bpp / 8] = r;
		img->data[y * img->line + x * img->bpp / 8 + 1] = g;
		img->data[y * img->line + x * img->bpp / 8 + 2] = b;
		img->data[y * img->line + x * img->bpp / 8 + 3] = a;
	}
}
